...
module.exports = {
	...
	execute(message) {
		...
		if (message.content && message.content.startsWith(prefix + "istqb")) {
			if (extractedArgs.length > 2) {
				return message.reply(
					`usage of !istqb : ${message.client.commands.get("istqb").usage} `,
				);
			}
        ...
        questions[getList[(getList.length * Math.random()) << 0]];
			...
			message.reply(
				`You must have 'active member' role to use this command. Participate in servers events to gain this role.`,
			);
		}
	},
};
